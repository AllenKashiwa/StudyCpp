...
反向遍历
*/
void ReTraverseListCore(LinkList L, void(*visit)(ElemType))
{
    if (L->next)
        ReTraverseListCore(L->next, visit);
    if (L && L->data)
        visit(L->data);
}

void ReTraverseList(LinkList L, void(*visit)(ElemType))
{
    // 反向遍历要去掉没有数据的头结点
    ReTraverseListCore(L->next, visit);
}
